---
name: corpus-pptx-v3
description: "Create PowerPoint decks (.pptx) through a three-stage flow: (1) collect & summarize the user's local source materials into a structured brief, (2) draft an iterative storyboard with the user — each planned slide bound to a concrete reference slide from the firm's tagged corpus via the slide-corpus MCP connector, (3) generate the .pptx by cloning the bound reference slides' RAW .pptx (via get_slide_pptx) and editing them in place. Use whenever the user asks to build, draft, or design a deck, slides, pitch, or presentation from source materials (RFP + supporting files). Requires the slide-corpus MCP connector to be enabled."
license: Proprietary. LICENSE.txt has complete terms
---

# Corpus-grounded PPTX Skill — 3-Stage Flow (v3: raw-slide cloning)
...
2. **Storyboard** the deck with the user — pick the closest reference deck for the
   ...
   `storyboard.json`.
3. **Generate** the `.pptx` by **cloning the bound reference slide's *raw* `.pptx`**
   (fetched with `get_slide_pptx`) and editing the real shapes in place — then QA.
...
`suggest_outline` / `match_slide` return weak or empty results, or when the bound
reference slide has no raw `.pptx` available.

> **What changed from v2.** v2's Stage 3 *reconstructed* each slide from the tag
> summary (`zones` / `slot_types_present`). v3 instead pulls the **real slide bytes**
> via `get_slide_pptx(deck, index)` and clones the actual shapes — far higher
> fidelity (exact geometry, fills, fonts, charts, grouped logos). The tag summary
> becomes the *checklist* for editing the clone, not the blueprint for rebuilding it.
> When `get_slide_pptx` returns null (deck not bundled / index unaligned), v3 falls
> back to the v2 tag-based reconstruction for that slide.
...
> `get_deck_assets`, `get_slide`, **`get_slide_pptx`**, `search_slides`,
> `find_similar_slides`, `find_slide_templates`, `get_house_style`, `start_deck`,
> **`suggest_outline`**, **`match_slide`**, `corpus_stats`. If they aren't available,
> tell the user to add the connector (see the server's `docs/POC.md`) before
> proceeding. `get_slide_pptx` requires PoC server v3+.
...
> **Bias bindings toward decks with raw slides where possible.** Cloning fidelity
> is highest when the bound `reference.deck` has a raw `.pptx` bundled (so
> `get_slide_pptx` works). If two candidate references tie on score and one is from
> a raw-available deck, prefer it. (You don't need to pre-check every binding — a
> null from `get_slide_pptx` in Stage 3 falls back cleanly.)
...
### Stage 3 — Generate the `.pptx` by cloning the bound RAW slides

The binding work is done. This stage clones the real reference slides and edits them.
...
3. **Per slide — clone the raw reference, then edit:**
   1. Call **`get_slide_pptx(reference.deck, reference.index)`**.
   2. **If it returns a slide** (`{base64, filename, …}`): `base64.b64decode` the
      `base64`, write it to a temp `.pptx`, and open it with `python-pptx`. This
      one-slide deck *is* the reference — it carries the real shapes, exact EMU
      geometry, fills, fonts, chart XML, tables, and grouped/branding shapes. **Build
      the new slide by cloning that slide and editing in place**, not by rebuilding
      from tags:
      - Copy the reference slide into your output deck (clone its shape tree), then
        **replace the text/data** with this slide's `intended_main_message` +
        `content_points`. Keep the donor's layout, positions, and styling.
      - Use `get_slide(reference.deck, reference.index)` as the **edit checklist**:
        its `zones` / `slot_types_present` tell you which shapes are the action
        title, the chart, the side callout, the footer — i.e. *what to fill where*.
      - Swap chart/table *values* for the new content while preserving the chart
        type, palette, and formatting from the donor.
   3. **If it returns null** (deck not bundled, or the tagged index doesn't align
      with the raw `.pptx`): fall back to the **v2 tag-based reconstruction** for
      that slide — build from `get_slide(...)`'s `zones` + `slot_types_present` in
      the locked design system. Note to the user which slides used the fallback.
4. **Apply the design system uniformly** (whether cloned or reconstructed):
   ...
   - Reproduce `recurring_elements` on every slide (footer text, page numbers,
     ...
     returned, fall back to the footer text `value`. (When you cloned the raw
     slide, the logo may already be present — don't double-place it.)
...
For the mechanical "how to clone/edit/write the .pptx" (python-pptx shape copying,
unpack/repack, templates), use your standard PPTX tooling; if the full Anthropic
`pptx` skill is loaded, follow its `editing.md` / `pptxgenjs.md`. This skill governs
*what* to build (storyboard-bound design + raw-slide cloning), not the file mechanics.
...
- **Content:** `python -m markitdown output.pptx` — check for missing content,
  ...
  and — critically for cloned slides — **leftover donor text/numbers** you forgot to
  replace.
...
- **Structure match:** for each generated slide, verify its structure matches its
  **bound reference's** `zones` and `slot_types_present` (re-fetch with
  `get_slide(reference.deck, reference.index)`). For cloned slides this should hold
  by construction — flag any drift.
- **Design fidelity match:** fonts / palette / footer / logo match the
  **snapshotted `reference_deck.design_system`** and `inferred_rules`
...
`list_vocabulary()`. Distill the brief; confirm with the user; write `storyboard.json`.

**Stage 2 — Storyboard.**
1. `suggest_outline(...)` → picks `nigeria-economic-outlook-october-2023-v1`
   (match_score 8/9, above threshold); snapshot its `design_system` + `inferred_rules`.
2. For each planned slide, `match_slide(..., prefer_deck="nigeria-economic-outlook-october-2023-v1")`
   → bind `reference {deck, index}` to the top match.
3. Render the markdown table; user edits; re-call `match_slide` for changed slides;
   mark `approved`; write `storyboard.json`.

**Stage 3 — Generate (raw clone).** Load `storyboard.json`. Fetch
`get_deck_assets("nigeria-economic-outlook-october-2023-v1")` for logos. Per slide:
`get_slide_pptx("nigeria-economic-outlook-october-2023-v1", index)` → decode, open
the one-slide deck, **clone its shapes**, replace the title/body/chart data with the
planned message + content_points (using `get_slide(...)` zones as the edit
checklist), keep the Arial red/black system, footer, and logo. QA loop until clean.

---

## Fallback (when the corpus has no close precedent, or no raw slide)

- **`get_slide_pptx` returns null** for a bound reference (deck not bundled, or the
  tagged index doesn't align with the raw `.pptx`): reconstruct that slide the v2
  way — build from `get_slide(...)`'s `zones` + `slot_types_present` in the locked
  design system. Tell the user which slides used the fallback.
...
- The **slide-corpus MCP connector** (required — the design/reference source).
  Must expose `get_slide_pptx`, `suggest_outline`, and `match_slide` (PoC server v3+).
- `pip install "markitdown[pptx]"` — text extraction · `pip install Pillow` —
  thumbnails · python-pptx (required — clone/edit the raw reference slides) or
  `npm i -g pptxgenjs` — building · LibreOffice (`soffice`) + Poppler (`pdftoppm`)
  — render to images for QA.

---

## Notes on raw-slide coverage

`get_slide_pptx` only serves decks whose source `.pptx` is bundled **and** whose
tagged slide indices align 1:1 with that `.pptx`. Decks that are tags-only (not
bundled, or index-misaligned) return null and use the v2 fallback. Treat a null as
normal, not an error — just disclose which slides it affected.
