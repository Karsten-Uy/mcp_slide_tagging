---
name: corpus-pptx-v6
description: "Create PowerPoint decks (.pptx) through a three-stage flow: (1) distill a brief, (2) storyboard against the firm's tagged corpus via the slide-corpus MCP connector — using corpus_stats coverage to keep the plan to slide kinds the corpus can actually clone — binding each slide to a concrete reference, (3) generate by CLONING the bound reference slide's raw .pptx (get_slide_pptx) and overwriting ONLY the visible text — using the per-shape map that endpoint now returns to target each edit — never restyling, then gating each repeated-element set through a three-axis consistency check before render. Use whenever the user asks to build, draft, or design a deck, slides, pitch, or presentation from source materials. Requires the slide-corpus MCP connector."
license: Proprietary. LICENSE.txt has complete terms
---

# Corpus-grounded PPTX Skill — v6 (clone-and-overwrite, shape-map-targeted, consistency-gated)
...
text** — targeting each edit with the **per-shape map** `get_slide_pptx` returns.

> **What changed from v5 — use the server's shape map + coverage.** Two endpoint
> capabilities the older flow worked around are now first-class:
> 1. **`get_slide_pptx` returns a `shapes` map** — one entry per text shape with
>    `shape_idx`, the current `text`, and its style/geometry signature (fill, bold,
>    size_pt, font, algn, anchor, width, height). So you no longer reverse-engineer which
>    shape holds the title vs a tile label by eyeballing the consistency-checker output:
>    the server tells you. Use it to (a) pick the exact `shape_idx`/run to overwrite,
>    (b) read the **donor ground-truth signature** for each repeated set (the gate's
>    target), and (c) get each slot's donor text length for the per-slot copy budget — all
>    without re-introspecting the donor yourself. (`shape_idx` is sparse: non-text shapes —
>    charts, pictures, tables, groups — aren't listed; reach those via python-pptx on the
>    bytes.)
> 2. **`corpus_stats` reports slide-kind coverage** (`by_message_type`,
>    `by_dominant_visual_element` alongside `by_slide_purpose`). Call it once in Stage 2 to
>    see which slide kinds the corpus can clone *before* you commit the storyboard, so an
>    unbindable slide is designed out up front instead of surfacing as a Stage-3 hard stop.
...
**First, pre-flight coverage.** Call `corpus_stats()` once and read `by_slide_purpose` /
`by_message_type` / `by_dominant_visual_element`. These tell you which slide kinds the
corpus can actually clone. Steer the storyboard toward well-covered kinds, and for any
brief point that only maps to a thin/zero-count kind, decide *now* (with the user) to
recast it onto a covered kind or drop it — rather than discovering at Stage 3 that it
can't bind. (This is the cheap, up-front version of the Stage-3 hard stop.)
...
3. **Per slide — clone the raw reference, then edit text:**
   1. `get_slide_pptx(reference.deck, reference.index)`. The response is
      `{base64, shapes, …}`: `base64` is the one-slide donor deck; `shapes` is the
      **edit map** — a list of `{shape_idx, text, fill, bold, size_pt, font, algn, anchor,
      width, height}`, one per text shape, in shape order.
   2. **If it returns a slide:** decode the base64, open the one-slide deck with
      ...
      **Use the `shapes` map to target each edit:** match each storyboard `content_point`
      to a `shape_idx` by its current `text` and role — the action title is typically the
      largest `size_pt` near the top; a tile label is small + bold/white; body is the
      lower-left block — then overwrite the run(s) of *that* `shape_idx` only. The map's
      style fields (`fill`/`bold`/`size_pt`/`font`/`algn`/`anchor`/`width`/`height`) are
      the **donor ground-truth signature** for the consistency gate: a repeated set
      (e.g. four tile labels) should already share one signature in the map — that shared
      value is the target your edited output must still match. Cross-check against
      `get_slide(reference.deck, reference.index)`'s `zones` / `slot_types_present` for the
      slide's intended structure; fill those, leave styling/geometry as the donor set it.
      **Write to the donor's footprint.** The per-slot copy budget comes straight from the
      map: keep each edited slot within about 1.0 to 1.2x of its `shapes[].text` length.
      Donor boxes
...
2. **Consistency gate (per repeated set, BEFORE rendering).** For every set of repeated
   ...
   You already have each set's `shape_idx` values and its **target** signature from the
   `get_slide_pptx` `shapes` map, so pass the groups directly and confirm the output still
   matches the donor's shared value (not just that the siblings agree with each other).
...
4. **Text-fit / overflow gate (BEFORE rendering).** Code can predict most overflow without
   ...
   accidental multi-run write). The per-slot donor lengths are already in the
   `get_slide_pptx` `shapes` map (budget at edit time from there); the checker below
   re-confirms at gate time and adds the geometry/autofit context. Pass the donor
   one-slide deck so it can compare lengths:
   ```bash
...
- The **slide-corpus MCP connector** — must expose `get_slide_pptx` **returning the
  `shapes` map** and `corpus_stats` **with slide-kind coverage** (`by_message_type` /
  `by_dominant_visual_element`). If `shapes` is absent the connector predates v6; fall
  back to introspecting the donor `.pptx` yourself (the v5 flow).
